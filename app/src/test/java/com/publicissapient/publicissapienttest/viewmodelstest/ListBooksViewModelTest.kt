...
class ListBooksViewModelTest {
    ...
    private lateinit var booksObserver: Observer<Resource<Books>>
    ...
    private val successResource = Resource.success(Books(books))
    ...
    fun setUp() {
        ...
        runBlocking {
            whenever(booksRepository.getBooks()).thenReturn(successResource)
        }
        viewModel = ListBooksViewModel(booksRepository)
        booksObserver = mock()
    }
    ...
    fun `when getWeather is called with valid location, then observer is updated with success`() =
        runBlocking {
            viewModel.book.observeForever(booksObserver)
            delay(10)
            verify(booksRepository).getBooks()
            verify(booksObserver, timeout(50)).onChanged(Resource.loading(null))
            verify(booksObserver, timeout(50)).onChanged(successResource)
        }
}